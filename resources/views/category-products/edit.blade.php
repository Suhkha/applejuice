...
@section('content')
    ...
    <form action="{{ route('category-products.update', $category->id) }}" method="POST" class="svelfit-form" data-parsley-validate="" enctype="multipart/form-data">
        @csrf
        @method('PUT')

        <div class="grid grid-cols-1 mt-5 mx-3 md:mx-7">
            <label class="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold">Nombre</label>
            <input class="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-teal-300" type="text" name="name" placeholder="Nombre" value="{{ $category->name }}" required/>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8 mt-5 mx-3 md:mx-7">
            ...
            <div class="grid grid-cols-1">
                ...
                <input class="w-full px-4 py-2 mt-2" type="file" name="cover" placeholder="Portada" required/>
            </div>
        </div>

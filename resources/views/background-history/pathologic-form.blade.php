...
@section('content')
    @include('partials.hero-form', ['sectionTitle' => 'Antecedentes patológicos'])
        ...
        @foreach ($pathologics as $pathologic)
            <div>
                <div class="grid grid-cols-1 mt-5 mx-3 md:mx-7">
                    <input type="hidden" name="pathologicIDS[]" value="{{ $pathologic->id }}">
                </div>

                <div class="grid grid-cols-1 mt-5 mx-3 md:mx-7">
                    <label class="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold"><span class="font-bold underline text-teal-500">{{ $pathologic->name }}</span> </label>
                    {{-- unstyled --}}

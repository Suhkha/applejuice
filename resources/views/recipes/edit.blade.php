...
@section('content')
    ...
    <form action="{{ route('recipes.update', $recipe->id) }}" method="POST" class="form-serialize">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 mt-5 mx-7">
            ...
            <textarea class="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-teal-300" name="ingredients" placeholder="Ingredientes" value="{{ $recipe->ingredients }}">{{ $recipe->ingredients }}</textarea>
        </div>

        <div class="grid grid-cols-1 mt-5 mx-7">
            <label class="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold">Preparación</label>
            <textarea class="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-teal-300" name="preparation" placeholder="Preparación" value="{{ $recipe->preparation }}">{{ $recipe->preparation }}</textarea>
        </div>

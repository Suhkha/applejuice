...
@section('content')
    ...
    <form action="{{ route('background.update', $background->id) }}" method="POST">
        ...
        <div class="grid grid-cols-1 mt-5 mx-7">
            ...
            <textarea class="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-teal-300" name="details" placeholder="Detalles" value="{{ $background->details }}">{{ $background->details }}</textarea>
        </div>

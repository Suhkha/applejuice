...
@section('content')
    @include('partials.hero-form', ['sectionTitle' => 'Editar recomendaciones'])

    <form action="{{ route('recommendation.update', $recommendation->id) }}" method="POST" class="svelfit-form"  >
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 mt-5 mx-3 md:mx-7">
            ...
            <textarea class="styled w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-teal-300" name="recommendation" placeholder="Recomendaciones" value="{{ $recommendation->recommendation }}"  >{{ $recommendation->recommendation }}</textarea>
        </div>

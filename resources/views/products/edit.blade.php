...
@section('content')
    ...
    <form action="{{ route('products.update', $product->id) }}" method="POST" class="form-serialize" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 mt-5 mx-7">
            ...
            <textarea class="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-teal-300" name="comments" placeholder="Comentarios" value="{{ $product->comments }}">{{ $product->comments }}</textarea>
        </div>

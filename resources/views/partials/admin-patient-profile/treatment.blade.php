        ...
        <div class="relative w-full max-w-full flex flex-grow flex-1">
            ...
            @if (!isset($treatment))
                <div class="relative w-full px-0 md:px-4 max-w-full flex-grow flex-1 text-right">
                    <a href="{{ route('treatment', ['user_id' => $userDetail->user_id, 'type' => 'profile']) }}" class="bg-white text-teal-400 text-xs font-bold uppercase md:px-6 px-3 py-2 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150">Nuevo tratamiento</a>
                </div>
            @endif
        </div>
    </div>
